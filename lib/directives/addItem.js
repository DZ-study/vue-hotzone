...
export default {
  beforeMount: function (el, binding, vnode) {
    ...
    console.log('addItems_context: ', context)

    function handleMouseDown (e) {
      context.mouseStatus = 'mousedown'
      e && e.preventDefault()
      ...
      function handleMouseUp () {
        ...
        if (context.isOverRange()) {
          context.overRange()
        } else if (container.height < MIN_LIMIT && itemInfo.width > MIN_LIMIT) {
          context.changeItem(Object.assign(perInfo, {
            topPer: 0,
            heightPer: 1
          }))
          ...
          context.changeItem(perInfo)
        } else {
          context.eraseItem()
        }
        context.mouseStatus = 'mouseup'
